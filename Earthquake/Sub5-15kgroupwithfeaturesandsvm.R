...
svm0 <- e1071::svm(meantime ~ ., data=tr1)
...
if (F) {
  ...
  testsubmissionpath <- paste0(folderpath,"Sub5-svm.csv")
  ...
  for (testfile in list.files(testfolderpath)) {
    ...
    # N <- nrow(testdf)
    testpred <- predict(svm0, data.frame(meanacoustic=mean(testdf$V1),
    ...
    testpred <- pmax( 0.05, testpred)
    testpred <- pmin(16, testpred)
